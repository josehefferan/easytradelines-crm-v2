...
const ContractSignaturePopup = ({ isOpen, onClose, brokerData, onSignComplete, currentUser }) => {
  ...
  const downloadPDF = () => {
    ...
    const addText = (text, fontSize = 10, fontStyle = 'normal') => {
      ...
      yPosition += 3; // Espaciado adicional
    };

    // Título del documento
    pdf.setFontSize(16);
    pdf.setFont('helvetica', 'bold');
    pdf.text('EASY TRADELINES RESELLER AGREEMENT', pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 15;
    ...
    // Firma del Reseller
    addText('RESELLER:', 10, 'bold');
    yPosition += 10;
    
    if (signatureData) {
      pdf.addImage(signatureData, 'PNG', margin, yPosition, 60, 20);
    }
    yPosition += 25;
    
    addText(`Printed Name: ${brokerData.first_name} ${brokerData.last_name}`);
    addText(`Date: ${getCurrentDate()}`);
    yPosition += 10;

    // Firma del Admin
    addText('EASY TRADELINES (Smart Latinos Consulting Group, DBA):', 10, 'bold');
    
    if (adminSignature) {
      pdf.addImage(adminSignature, 'PNG', margin, yPosition, 60, 20);
    }
    yPosition += 25;
    
    addText(`By: ${adminName || '___________________________'}`);
    addText(`Date: ${getCurrentDate()}`);
    addText(`Initial: ${adminInitials || '____'} I have understood the document.`);

    // Descargar el PDF
    pdf.save(`Easy_Tradelines_Reseller_Agreement_${brokerData.first_name}_${brokerData.last_name}_${getCurrentDate().replace(/,/g, '').replace(/\s/g, '_')}.pdf`);
  };
              ...
              <div style={styles.signatureCol}>
                ...
                {!isSigned ? (
                  ...
                  <div style={styles.signedIndicator}>
                    <Check size={16} />
                    Signed by {brokerData.first_name} {brokerData.last_name}
                  </div>
                )}
                
                <div style={{marginTop: '12px', fontSize: '11px', color: '#6b7280', textAlign: 'center'}}>
                  <p><strong>Printed Name:</strong> {brokerData.first_name} {brokerData.last_name}</p>
